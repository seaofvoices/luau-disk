...
local insertBefore = require('../insertBefore')

local expect = jestGlobals.expect
local it = jestGlobals.it

it('inserts before first matching element', function()
    local result = insertBefore({ 1, 2, 3, 4 }, 99, function(x)
        return x == 2
    end)
...
it('inserts before first element when match is at beginning', function()
    local result = insertBefore({ 1, 2, 3, 4 }, 99, function(x)
        return x == 1
    end)
...
it('returns original array when no match is found', function()
    ...
    local result = insertBefore(original, 99, function(x)
        return x == 5
    end)
...
it('uses first match when multiple elements match', function()
    local result = insertBefore({ 1, 2, 2, 3 }, 99, function(x)
        return x == 2
    end)

    expect(result).toEqual({ 1, 99, 2, 2, 3 })
end)

it('works with start parameter to limit search', function()
    local result = insertBefore({ 1, 2, 3, 2, 4 }, 99, function(x)
        return x == 2
    end, 3)

    expect(result).toEqual({ 1, 2, 3, 99, 2, 4 })
end)

it('returns original array when no match after start parameter', function()
    local original = { 5, 1, 2, 3 }
    local result = insertBefore(original, 99, function(x)
        return x == 1
    end, 3)

    expect(result).toEqual({ 5, 1, 2, 3 }) -- No match from index 3 onwards
    expect(result).toBe(original)
end)
...
it('calls predicate with correct arguments', function()
    ...
    insertBefore({ 10, 20, 30 }, 99, function(x)
...
it('stops calling predicate after first match is found', function()
    ...
    insertBefore({ 1, 2, 2, 3 }, 99, function(x)
        callCount = callCount + 1
        return x == 2
    end)
...
it('works with nil start parameter', function()
    local result = insertBefore({ 1, 2, 3 }, 99, function(x)
        return x == 2
    end, nil)
